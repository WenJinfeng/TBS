...
def function_handler(request):
    ...
    if model is None:
        download_blob(bucket, 'fashion_mnist_weights.index', '/tmp/fashion_mnist_weights.index')
        download_blob(bucket, 'fashion_mnist_weights.data-00000-of-00001', '/tmp/fashion_mnist_weights.data-00000-of-00001')
        model = CustomModel()
        model.load_weights('/tmp/fashion_mnist_weights')

    download_blob(bucket, 'test.png', '/tmp/test.png')
    start = time()
